...
public class PlayerOne extends Player {
	...
	AnimatedSprite down = new AnimatedSprite(SpriteSheet.player_down, 32, 32, 3);
	AnimatedSprite up = new AnimatedSprite(SpriteSheet.player_up, 32, 32, 3);
	AnimatedSprite left = new AnimatedSprite(SpriteSheet.player_left, 32, 32, 3);
	AnimatedSprite right = new AnimatedSprite(SpriteSheet.player_right, 32, 32, 3);
	AnimatedSprite hurt = new AnimatedSprite(SpriteSheet.player_hurt, 32, 32, 1);
	
	AnimatedSprite animSprite = down; // Holds the current animation of the player, set to down as default
	AnimatedSprite temp;
	
	static UIPanel panel = (UIPanel) new UIPanel(new Vector2i(0, 330 * 3), new Vector2i(180 * 3, 20 * 3), true).setColour(0x7f95a5a6); // Creates a panel on the UI at the bottom of the screen
	
	public PlayerOne(String id, int x, int y, Keyboard input) { // Spawns the player one at the given coordinates
		super(id, x, y, panel);
		sprite = Sprite.player_down; // The players default sprite
		this.input = input;
	}
	
	public void update() {
		if (health <= 0) Game.oneAlive = false;
		super.update();
	...
	public void updateDynamite() { // Checks for dynamite placement
		if(dynamiteNumber != 0) { // If the player has dynamite available to place
			if (input.onePlace && currentPlace <= 0) { // And the player has pressed the place button and the place rate allows it
	...
	public void updatePowerup() { // Checks if a powerup should be used
		if (input.oneActivate && !used && time %  10 == 0) {
			used =  true;
			onMenu = false;
		}
	}
