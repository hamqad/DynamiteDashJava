...
public class ShooterProLeft extends Mob {
	
	// Animation of chasers each direction
	private AnimatedSprite down = new AnimatedSprite(SpriteSheet.wizard_left, 32, 32, 1);
	private AnimatedSprite up = new AnimatedSprite(SpriteSheet.wizard_left, 32, 32, 1);
	private AnimatedSprite hurt = new AnimatedSprite(SpriteSheet.wizard_left, 32, 32, 1);
	...
	public ShooterProLeft(int x, int y) { // Spawns a shooter at the given position
		this.x = x << 4;
		this.y = y << 4;
		sprite = sprite.dummy;
	}
	...
	public void update() {
		...
		// Handles the damaging of the player:
